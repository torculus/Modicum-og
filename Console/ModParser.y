...
text:
    ...
    | LEFT TIMES text TIMES RIGHT   { YYABORT; } /* enables comments */
    | QUOTE text QUOTE              { $$ = $2; }
;
...
/* node *head; head = (node *)malloc(sizeof(node)); head->contents = $1; head->next = NULL; */

list:
    LBRACK sequence RBRACK          { size = myCount(argsList); for (int i=0; i<10; i++) {L[i] = argsList[i]; } }
    | WSPACE list                   { $$ = $2; }
    | list WSPACE                   { $$ = $1; }
